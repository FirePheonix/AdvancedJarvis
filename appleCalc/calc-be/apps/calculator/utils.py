...
def analyze_image(img: Image, dict_of_vars: dict):
    dict_of_vars = json.dumps(dict_of_vars)
    #we serialized the dictionary of variables to a string using json.dumps so that I can pass it in the prompt easily.
    prompt = (   
    ...
    # Generate content using a model with the given prompt and image
    response = model.generate_content([prompt, img])  
    # Print the generated text response from the model
    print(response.text)

    # Initialize an empty list to store parsed answers
    answers = []  

    try:
        # Try to parse the text response into a Python object using ast.literal_eval
        answers = ast.literal_eval(response.text)  
    except Exception as e:
        # If parsing fails, print an error message indicating the issue
        print(f"Error in parsing response from Gemini API: {e}")  

    # Print the parsed answers (or an empty list if parsing failed)
    print('returned answer ', answers)  

    # Iterate over each answer in the parsed answers
    for answer in answers:
        # Check if the answer dictionary contains the key 'assign'
        if 'assign' in answer:
            # If 'assign' exists, set its value to True
            answer['assign'] = True  
        else:
            # If 'assign' doesn't exist, add it with the value False
            answer['assign'] = False  

    # Return the modified list of answers, where each now has an 'assign' key
    return answers
